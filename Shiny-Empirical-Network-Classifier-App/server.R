...
server <- shinyServer(function(input, output, session) {
  trained_model <- readRDS("final.fitted.model.new.rds")
    ...
    predictions <- eventReactive(input$classify, {
      ...
      data <- data %>%
        ...
        select(
          graph_name, NumOfNodes, edges, mean_eccentr, mean_path_length,
            ...
            replace_names <- function(old_names, new_names2, row_names) {
              for (i in seq_along(old_names)) {
                for (j in seq_along(old_names)) {
                  if (i != j) {
                    old_pair <- paste(old_names[i], old_names[j], sep = ":")
                    new_pair <- paste(new_names2[i], new_names2[j], sep = ":")
                    row_names <- gsub(old_pair, new_pair, row_names)
                  }
                }
              }
              return(row_names)
